...
class Trace:
    def __init__(
        ...
        """Initialize a Trace helper for logging and optional trace extension.

        Parameters:
            name: The base name for trace events.
            logger: Logger used for debug-level logging.
            request: Optional Request used to obtain a 'trace' extension callback.
            kwargs: Optional keyword arguments that will be included with the
                'started' trace event.
        """
        self.name = name
        self.logger = logger
    ...
    def trace(self, name: str, info: dict[str, typing.Any]) -> None:
        """Invoke a synchronous trace callback (if present) and/or log a debug message.

        The trace extension callback, when present, is expected to be a normal
        synchronous function. If it returns a coroutine, a TypeError is raised.

        Parameters:
            name: The event name to send to the extension / logger.
            info: A mapping of event data to send.
        """
    ...
    def __enter__(self) -> Trace:
        """Enter the tracing context, emitting a 'started' event if tracing is enabled."""
    ...
    def __exit__(
        ...
        """Exit the tracing context, emitting 'complete' or 'failed' event as appropriate."""
        if self.should_trace:
